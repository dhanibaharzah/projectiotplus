 ...
 */
class Hse_jsa extends BaseController
{
    /**
		...
		if(!empty($record)){
			...
			if($record->panas != 2 and (!empty($record->panas))){
				...
				$a .=	$record->id.'"target="_blank" class="btn-sm btn btn-block btn-danger">';
					...
					$a .= '<span '; 
					if($record->panas != 1){ 
						$a .= 'class="label label-default"';}else{
						$a .= 'class="label label-danger"';}
						$a .= '>'; 
						if($record->panas == 1){ $a .= '(Rejected)';}
			...
			if($record->terbatas != 2 and (!empty($record->terbatas))){
				...
				$a .= 'Confined Space ';
				$a .= '<span '; 
				if($record->terbatas != 1){ 
					$a .= 'class="label label-default"';}else{
					$a .= 'class="label label-danger"';}
					$a .= '>';
						if($record->terbatas == 1){ $a .= '(Rejected)';}
			...
			if($record->penggalian != 2 and (!empty($record->penggalian))){
				...
				$a .=	$record->id.'"target="_blank" class="btn-sm btn btn-block btn-primary">';
					...
					$a .= 'Digging ';
					$a .= '<span '; 
					if($record->penggalian != 1){ 
						$a .= 'class="label label-default"';}else{
						$a .= 'class="label label-danger"';}
						$a .= '>';
						if($record->penggalian == 1){ $a .= '(Rejected)';}
			...
			if($record->listrik != 2 and (!empty($record->listrik))){
				...
				if($record->listrik != 1){ 
					...
					$a .= 'class="label label-danger"';}
					$a .= '>';
						if($record->listrik == 1){ $a .= '(Rejected)';}
			...
			if($record->general != 2 and (!empty($record->general))){
				...
				if($record->general != 1){ 
					...
					$a .= 'class="label label-danger"';}
					$a .= '>'; 
						if($record->general == 1){ $a .= '(Rejected)';}
    ...
    function create()
    {
		...
		if($this->vendorId >= 40000 AND $this->vendorId <= 49999){
			...
			$data['listnamecon'] = $this->hse_jsa_model-> get_listnamecon($this->vendorId);
		}
		if($this->vendorId >= 90000 AND $this->vendorId <= 99999){
			$data['company_name'] = 'PT SLCI';
			$data['userList'] = $this->hse_jsa_model->get_phone();
		}
	...
	function notifmyjsa(){
		$searchText = $this->security->xss_clean($this->input->post('searchText'));
		$notifappjsa = $this->hse_jsa_model->get_myjsaCount($this->name, $searchText);
		echo $notifappjsa;
	}
	...
	function closedjsa(){
		...
		if($this->vendorId >= 40000 AND $this->vendorId <= 49999){
			...
			echo $this->name.'-'.$start.'-'.$end.'-'.$lokasi.'-'.$type1.'-'.$type2.'-'.$type3.'-'.$type4.'-'.$type5.'-'.$type6.'-'.$searchText.'-'.$returns["page"].'-'.$returns["segment"];
	...
	function edit_convendor(){
		...
		$updating = $this->hse_jsa_model->edit_convendor($update, $id);
		redirect('vendorinduction');
	}

	function edited_convendor($id){
			$role = $this->hse_jsa_model->get_userbyuname($this->name);
			if($role->hse_role == 9 or $role->hse_role == 10){
				$data['convendor_data'] = $this->hse_jsa_model->get_convendor($id);
				$this->global['pageTitle'] = 'RAWR : Vendor Penalty';
				$this->loadViews("hse_jsa/inductionedit", $this->global, $data, NULL);
				}
			else{
				$this->loadThis();
			}
	...
	function vendorinduction(){
		...
		$data['venrole'] = $this->hse_jsa_model->get_userbyuname($this->name);
		$searchText = $this->security->xss_clean($this->input->post('searchText'));
		$data['searchText'] = $searchText;
		$this->load->library('pagination');
		...
		$returns = $this->paginationCompress ( 'vendorinduction/', $count, 10);
	...
	function penalty_list($regnums){
				$data['list_penalty'] = $this->hse_jsa_model->penalty_list($regnums);
				$data['penalty_name'] = $this->hse_jsa_model->penalty_name($regnums);
				$this->global['pageTitle'] = 'RAWR : Vendor Penalty';
				$this->loadViews("hse_jsa/inductionpenview", $this->global, $data, NULL);
			}
	...
	function appjsa(){
		if($this->vendorId > 90000)
		{
			...
			$returns = $this->paginationCompress ( "appjsa/", $count, 10 );
			...
			$this->loadViews("hse_jsa/appjsa", $this->global, $data, NULL);
		}else{
			$this->loadThis();
		}
	}

	function notifappjsa(){
		$hse_role = $this->hse_jsa_model->get_userbyuname($this->name);
		$fromDate = $this->input->post('fromDate');
		$toDate = $this->input->post('toDate');
		$type1 = $this->input->post('type1');
		...
		$notifappjsa = $this->hse_jsa_model->get_appjsanotif($start, $end, $lokasi, $status, $type1, $type2, $type3, $type4, $type5, $type6, $this->name,$hse_role->hse_role, $hse_role->hse_picar);
		echo $notifappjsa;
	}
	...
	function qrjsatoday(){
		...
		if($this->vendorId > 90000){
			$count = $this->hse_jsa_model->get_todayjsaCount($today_unix,$searchText);
			$returns = $this->paginationCompress ( "today_jsa/", $count, 50 );
			$data['today_jsa'] = $this->hse_jsa_model->get_todayjsa($today_unix, $searchText, $returns["page"], $returns["segment"]);
		}else{
			$count = $this->hse_jsa_model->get_todayjsaxCount($today_unix,$this->name,$searchText);
			$returns = $this->paginationCompress ( "today_jsa/", $count, 50 );
		...
		$this->loadViews("hse_jsa/qrjsatoday", $this->global, $data, NULL);
	}

	function notiftodayjsa(){
		$notifappjsa = $this->hse_jsa_model->get_todayjsanotif($this->name);
		echo $notifappjsa;
	}
