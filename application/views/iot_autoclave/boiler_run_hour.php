...
<script>
	$('#run_chart').load('<?php echo base_url(); ?>iot_js_hour_boiler/<?php echo date('U'); ?>');
	...
	$('#datepick').on('apply.daterangepicker', function(ev, pickerx){
		$('#run_chart').load('<?php echo base_url(); ?>iot_js_hour_boiler/' + pickerx.startDate.format('X'));
	});
</script>