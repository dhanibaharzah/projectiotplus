			...
			<div class="col-lg-12 col-md-12 col-xs-12">
				...
				<script type="text/javascript">
					$(document).ready(function() {
						<?php $unix = date('U'); $unix = $unix - 86400; ?>
						$('#getdate<?php echo $record->bu_srmi; ?>').load('<?php echo base_url(); ?>line_srmi_view_chartv2/' + '<?php echo date('Y-m-d', $unix); ?>' + '/' + '<?php echo date('Y-m-d', $unix); ?>' + '/' + '<?php echo $record->bu_srmi; ?>' );
					});
				</script>
			</div>
	<?php 
		...
		<div class="col-lg-12 col-md-12 col-xs-12">
			<div class="box box-primary">
				<div class="box-body">
					...
					<script src="<?php echo base_url(); ?>assets/bootstrap/js/bootstrap.min.js" type="text/javascript"></script>
					<script type="text/javascript">
						$('#get_summary').load('<?php echo base_url(); ?>line_get_srmi_mtd/' + '<?php echo $unix; ?>' + '000/' );
					</script>
				</div>
			</div>
		</div>
			...
			<div class="col-lg-12 col-md-12 col-xs-12">
				<div class="box box-primary">
					<div class="box-body">
						<div id="get_map"></div>
						<script type="text/javascript">
							$('#get_map').load('<?php echo base_url(); ?>line_srmi_map/' + '<?php echo date('Y-m-d', $unix); ?>' + '/' + '<?php echo date('Y-m-d', $unix); ?>');
						</script>
					</div>
				</div>
			</div>
...
<script type="text/javascript">
	...
	function load_chartx(){
		...
		?>
			if(date1 != '' && date2 != ''){
				$('#getdate<?php echo $record->bu_srmi; ?>').load('<?php echo base_url(); ?>line_srmi_view_chartv2/' + date1 + '/'+ date2 + '/'+ '<?php echo $record->bu_srmi; ?>');
				$('#get_summary').load('<?php echo base_url(); ?>line_get_srmi_mtdv2/' + date2 );
