				...
				<div class="box box-primary">
					<div class="box-body no-padding">
						...
						<div class="col-lg-5 col-xs-12 direct-chat direct-chat-primary">
							<h3 class="fa fa-wechat">Execution Status</h3>
							<div class="direct-chat-messages" id="exe_prog" style="display: flex; flex-direction: column-reverse;"></div>
						</div>
					</div>
				</div>
			</div>
...
$('#calendar').fullCalendar({
     eventSources: [
         {
             events: function(start, end, timezone, callback) {
                 $.ajax({
                 url: '<?php echo base_url() ?>getprojectapp',
                 dataType: 'json',
                 data: {
	 ...
	 dayClick: function(date, jsEvent, view) {
		...
		$('#tabel').load('<?php echo base_url();?>gettabelapp/'+ date.format('X'));
		$('#header_button').load('<?php echo base_url();?>getheaderapp/'+ date.format('X'));
	 },
